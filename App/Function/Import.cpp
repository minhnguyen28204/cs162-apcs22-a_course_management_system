...
bool importListyear(const string& folderpath, DLinkedList <Year> &year_list)
{
    ...
    while (getline(fin, line))
    {
        ...
        string yearpath=folderpath+"/"+yearid+"-"+to_string(new_year.IDyear+1);
...
{
    string semester_file=folderpath +"/semesters"+"/semesters.CSV";
...
{
    string class_file=folderpath+ "/classes"+"/classes.CSV";
    ...
    while(getline(fin,line))
    {
        ...
        string classpath=folderpath+"/"+clname;
        if(!importListstudent(classpath,cur_course.stu_list))
        return false;
    }
    fin.close();
    return true;
